...
function listTitles(booksArray) {
  return booksArray.map(object => object.title);
}
...
function listAuthors(booksArray) {
  return booksArray.map(object => object.author);
}
...
function listValues(booksArray, key) {
  return booksArray.map(object => object[key]);
}
...
function getAvailableBooks(booksArray) {
  const someThing = booksArray.filter(object => object.isAvailable)
  return someThing.map(object => object.title);
}
